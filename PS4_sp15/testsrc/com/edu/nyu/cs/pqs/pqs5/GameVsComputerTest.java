...
public class GameVsComputerTest {
  ...
  public void testGetInstance() {
    GameVsComputer game = GameVsComputer.getInstance();
    assertTrue(game instanceof GameVsComputer);
  }
  ...
  @Test
  public void testComputerMoveGeneration(){
    GameVsComputer game = GameVsComputer.getInstance();
    GameState gameState = new GameState("Vs Computer");
    gameState.setBoard((short)0, (short)0, 'B');
    gameState.setBoard((short)0, (short)1, 'B');
    gameState.setBoard((short)0, (short)2, 'B');
    assertEquals(game.generateComputerMoveColumn(gameState),3);
    gameState.setBoard((short)0, (short)2, 'A');
    gameState.setBoard((short)0, (short)3, 'A');
    short column = game.generateComputerMoveColumn(gameState) ;
    assertTrue(column>=0 && column <=6);
    //The low scores array gets no coverage because Player A's pawns will 
    //never be in a winning combination with one look ahead. However if we
    //were to implement a 2 or higher level of lookahead, the low scores will
    //come in handy
  }
